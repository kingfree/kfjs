...
<?=_GB_PROBLEM_ID?>:<input type=text size=4 name=problem_id value='<?=$problem_id?>'>
<?=_GB_USER?>:<input type=text size=4 name=user_id value='<?=$user_id?>'>
<?if (isset($cid)) echo "<input type='hidden' name='cid' value='$cid'>";?>
<?=_GB_LANG?>:<select size="1" name="language">
<?
...
<?=_GB_RESULT?>:<select size="1" name="jresult">
<?
...
echo "<input type=submit value="._GB_SEARCH."></form>";
?>
</div>
<table align=center>
<tr  class='toprow'>
